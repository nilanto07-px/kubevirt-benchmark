...
Examples:
  ...
  %(prog)s --start 1 --end 50 --log-level DEBUG --log-file recovery.log
    ...
    # Test range
    parser.add_argument("-s", "--start", type=int, default=1, help="Start namespace index (default: 1)")
    parser.add_argument("-e", "--end", type=int, default=5, help="End namespace index, inclusive (default: 5)")

    # VM configuration
    parser.add_argument("-n", "--vm-name", type=str, required=True, help="VMI resource name to monitor")
    parser.add_argument(
        "--namespace-prefix",
        type=str,
    ...
    parser.add_argument("--ssh-pod", type=str, required=True, help="Pod name for ping tests")
    parser.add_argument("--ssh-pod-ns", type=str, required=True, help="Namespace of SSH pod")
    ...
    parser.add_argument(
        "--cleanup-vms", action="store_true", help="Also delete VMs and namespaces during cleanup (requires --cleanup)"
    )
    parser.add_argument("--far-name", type=str, help="Name of the FAR resource to clean up")
    parser.add_argument(
        "--far-namespace", type=str, default="default", help="Namespace of the FAR resource (default: default)"
    )
    parser.add_argument("--failed-node", type=str, help="Name of the node that was marked as failed (to uncordon)")
    parser.add_argument(
        "--dry-run-cleanup", action="store_true", help="Show what would be cleaned up without actually cleaning"
    )
    parser.add_argument("--yes", action="store_true", help="Skip confirmation prompt for cleanup (use with caution)")

    parser.add_argument(
        "--skip-ping", action="store_true", help="Skip ping recovery checks and only measure VMI Running time"
    )
...
def get_vmi_status_and_ip(vmi_name: str, namespace: str, logger) -> Tuple[str, str, str]:
    ...
    try:
        ...
        returncode, phase, _ = run_kubectl_command(
            ["get", "vmi", vmi_name, "-n", namespace, "-o", "jsonpath={.status.phase}"], check=False, logger=logger
        )
        phase = phase.strip() if returncode == 0 else ""

        # Get ready condition
        returncode, ready, _ = run_kubectl_command(
            ["get", "vmi", vmi_name, "-n", namespace, "-o", 'jsonpath={.status.conditions[?(@.type=="Ready")].status}'],
            check=False,
            logger=logger,
        )
...
def wait_for_ping_recovery(
    ns: str, vmi_name: str, ssh_pod: str, ssh_pod_ns: str, start_ts: datetime, poll_interval: int, logger
) -> Tuple[str, float]:
    """
...
def main():
    ...
    if args.skip_ping:
        print("  Ping checks skipped:       True")
    ...
    if args.cleanup or args.dry_run_cleanup:
        ...
        if not args.dry_run_cleanup and not confirm_cleanup(len(namespaces), args.yes):
            ...
            try:
                cleanup_stats = {"far_deleted": 0, "annotations_removed": 0, "nodes_uncordoned": 0, "errors": 0}

                # Remove FAR resource
                if args.far_name:
                    if args.dry_run_cleanup:
                        logger.info(
                            f"[DRY RUN] Would delete FAR resource: {args.far_name} in namespace {args.far_namespace}"
                        )
                ...
                if args.failed_node:
                    ...
                    elif uncordon_node(args.failed_node, logger):
