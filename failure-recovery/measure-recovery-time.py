...
def main():
    ...
    def output(msg=""):
        """Print to stdout and optionally log to file."""
        if logger:
            logger.info(msg)
        print(msg)
    ...
    output("\n" + "=" * 100)
    output(f"{'Namespace':<30}{'Time to Run(s)':<15}{'Time to Ping(s)':<15}{'IP at Run':<20}{'IP at Ping':<20}")
    output("-" * 100)
    ...
    for ns, ip_run, t_run, ip_ping, t_ping in sorted(results, key=lambda x: x[0]):
        ...
        output(f"{ns:<30}{run_str:<15}{ping_str:<15}{ip_run:<20}{ip_ping:<20}")
    ...
    # Print statistics
    output(f"\n{'Recovery Statistics:'}")
    output(f"  Total VMIs:                {len(results)}")

    if args.skip_ping:
        output(f"  Ping checks skipped:       True")
        output(f"  Successfully reached Running: {len(running_times)}")
        output(f"  Failed to reach Running:      {len(results) - len(running_times)}")
    else:
        output(f"  Successfully recovered (ping): {len(ping_times)}")
        output(f"  Failed to recover (ping):     {len(results) - len(ping_times)}")

    if running_times:
        output(f"\n  Time to Running:")
        output(f"    Average:                 {sum(running_times) / len(running_times):.2f}s")
        output(f"    Maximum:                 {max(running_times):.2f}s")
        output(f"    Minimum:                 {min(running_times):.2f}s")

    if ping_times:
        output(f"\n  Time to Ping:")
        output(f"    Average:                 {sum(ping_times) / len(ping_times):.2f}s")
        output(f"    Maximum:                 {max(ping_times):.2f}s")
        output(f"    Minimum:                 {min(ping_times):.2f}s")
